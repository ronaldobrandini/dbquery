...
include_once('sys/cfg.php');

$select = new \db\SqlSelect;

$sqlCriteria = new \db\SqlCriteria();    
$sqlCriteria->add(new \db\SqlFilter('idSetor', '=', 133));

$join = new \db\SqlJoin('INNER', 'perfilUsuario PU');
$join->addFilter('U.idPerfilUsuario', '=', 'PU.idPerfilUsuario');

$joins = array($join);

$promotor = new \datamapper\Promotor();
$result = $promotor->load($sqlCriteria, $joins);
